...
export default function ColorPicker({
  ...
  return (
    ...
    >
      ...
      <div className="flex gap-2 flex-wrap m-0 mt-3 p-0">
        {basicColors.map((basicColor) => (
          <Button
            key={basicColor}
            variant={basicColor === selfColor.hex ? 'default' : 'outline'}
            size="icon"
            className={cn(
              "size-5 overflow-hidden",
              basicColor === selfColor.hex && 'ring-2 ring-primary',
            )}
            onClick={() => {
      ...
      <MoveWrapper
        ...
        <div
          className={cn(
            "absolute size-5 rounded-full border-2 border-primary shadow -translate-2.5",
